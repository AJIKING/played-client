...
class PlayedSupabaseUser extends BaseAuthUser {
  PlayedSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> playedSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PlayedSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
